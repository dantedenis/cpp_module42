...
ClapTrap::~ClapTrap() {
	std::cout << "Called destructor [ClapTrap] for [" << this->getName() << "] object" << std::endl;
}
...
void	ClapTrap::takeDamage(unsigned int amount) {
	...
	if (this->damage < 0)
		this->damage = 0;
	std::cout << '[' << this->name + "]: My damage up to " << this->damage << std::endl;
}
...
void	ClapTrap::beRepaired(unsigned int amount) {
	...
	if (this->hit < 0)
		this->hit = 0;
	std::cout << '[' << this->name + "]: I repaired my hit to " << this->hit << std::endl;
}
